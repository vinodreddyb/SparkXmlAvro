...
object XmlToAvroLocal {

  def main(args: Array[String]): Unit = {
   System.setProperty("hadoop.home.dir", "C:/winutils");
   val sparkConf = new SparkConf().setAppName("SampleXML").setMaster("local")
    ...
    val xmlReadData = sparkContext.newAPIHadoopFile("troubleticket.xml", classOf[XMLInput], classOf[LongWritable], classOf[Text], conf)

    // Convert to xml to avro model objects
    var xml = xmlReadData.mapValues { x => 
            //println("xml--->" + x)
    ...
    nrdd.saveAsNewAPIHadoopFile("outbig", classOf[com.teliacompany.toolbox.model.Ticket],
  ...
  def convertFromXmlToAvroModel(ticket:com.telia.spark.generated.Ticket) : com.teliacompany.toolbox.model.Ticket = {
    ...
    val avroNodesType = xmlNodes.map { x =>
      var spits = x.affected.spid.map { spit =>
        SpidType(spit.value, spit.id.get)
      }.toList

      var productType = x.affected.product.map { product =>
        ProductType(product.value, product.typeValue.get)
      }.toList

      var avroAffectedType = AffectedType(spits, productType)
